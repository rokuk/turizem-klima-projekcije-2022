...
ASHRAE scale from skin temperature. See [CDS](https://cds.climate.copernicus.eu/cdsapp#!/dataset/sis-tourism-climate-suitability-indicators?tab=doc).
...
CIT 3S lookup table. See [CDS](https://cds.climate.copernicus.eu/cdsapp#!/dataset/sis-tourism-climate-suitability-indicators?tab=doc) and (De Freitas et al., 2008).
...
Other CIT lookup tables. For cloud cover a border of 50 % is used. See (Bafaluy et al., 2013).
...
Wind is in m/s, Tmax in degC, cloud cover in %, precipitation in mm and relative humidity in %.
...
Join relative humidity and daily datasets, calculate effective temperature and HCI from facet scores.
...
Split data into periods and join them with relevant columns, add period column:
...
Calculate number of days in each CIT category, set period, category and month names:
...
Calculate number of days in each HCI category, set period, category and month names:
...
Calculate mean effective temperature for each period and month, set period and month names:
...
```{r, fig.width=9, fig.height=3, dev='svg'}
for (station_name in station_names) {
    data <- calculate_hci_urban(
        prepare_RH_data(
            import_RH_data(station_name)), 
